...
function save_url()
{
    var url = "" , first_bit = 0, /* from left */ sum = 0, last_index,dirty_bits_value=0,dirty_bits_position=0;
    for (var i in opts) {
    ...
    for( var i in opts) {
        var obj = opts[i] , dirty_bits_current = 0;
		if(obj.dirty_bits){
		...
		if( obj.type === "bits"){
			var remainder = obj.get("url"), remainder_bits = obj.bits;
...
function load_url(url_query)
{
    ...
    var current_char_num = 0,first_bit = 0,dirty_bits_value=0,dirty_bits_position=0,dirty_bits_current; // 0 char is ?
    for (var i in opts) {
